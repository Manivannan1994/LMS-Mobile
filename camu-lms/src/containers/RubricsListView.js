...
const RubricsViewComponent = (props) => {
  ...
  const backToRubList = () => {
    history.push({ pathname: "/Rubrics-page", state: location.state });
  }
  ...
  const editRubrics = () => {
    if(props.aRubrics && props.aRubrics.length && ((props.aRubrics[0].asgnCnt && props.aRubrics[0].asgnCnt > 1) || (props.aRubrics[0].grdCnt && props.aRubrics[0].grdCnt > 0))){
      ...
      history.push({ pathname: "Rubrics-create", search:`?id=${rubrcId}`, state: location.state });
    }
  }
  ...
  return (
    <div className="rubrics-view_box dashboard-rubrics_block">
      {
        (!props.isFrmGrd && !props.isFrAsgn) && 
        <div className="rubrics-options">
          <div className="hide-content_view" onClick={() => history.push({ pathname: "/Rubrics-page"})}>
            ...
            {/* <UIPerWrapper perCode={["rp_can_create_or_edit_in_rubrics"]}> */}
              {
                ...
                <div className={((props.aRubrics[0].asgnCnt && props.aRubrics[0].asgnCnt > 1) || (props.aRubrics[0].grdCnt && props.aRubrics[0].grdCnt > 0)) && "tooltip-edit_ins_rubric"} style={{ display : ((props.aRubrics[0].asgnCnt && props.aRubrics[0].asgnCnt > 1) || (props.aRubrics[0].grdCnt && props.aRubrics[0].grdCnt > 0)) && "block"}} data-tooltip= {props.t("translate:RUBRIC_HOVER_INFO")}>
